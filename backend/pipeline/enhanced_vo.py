...
# --- TTS Manager (Modified for Direct Speech) ---
class TTSManager(threading.Thread):
    ...
    def run(self):
        ...
        while not self.stop_event.is_set():
            ...
            try:
                ...
                if instruction: # Check if not None (put by stop())
                ...
                if instruction is not None:
                    try:
                        # Try to mark done even if speaking failed mid-way
                ...
                time.sleep(0.5) # Wait briefly after error
    ...
    def stop(self):
        ...
        self.stop_event.set()
        self.queue.put(None) # Unblock waiting get()
        time.sleep(0.3)
        try:
...
def load_depth_model(device):
    try:
        model_type = "DPT_Hybrid" #"MiDaS_small" # Use smaller model if memory issues persist
...
def generate_instructions(waypoints, step_size=15, angle_threshold=40): # Increased threshold
    """
    Generates navigation instructions from waypoints.
    Note: Distances are in pixels relative to VO scale.
    """
    if not waypoints or len(waypoints) < 2:
        return ["No path computed"]

    instructions = []
    heading = np.array([0.0, -1.0]) # Initial heading: Upwards in image (negative Y)
    current_action = None
    count = 0
    ...
    for i in range(1, len(waypoints)):
        ...
        if abs(turn_angle) < angle_threshold:
            action = "Move forward"
            # Don't update heading if moving straight
        elif turn_angle > 0:
            action = "Turn left slightly and move"
            heading = vec_norm # Update heading after turn decision
        else:
...
def run_depth_estimation(midas, transform, image_rgb, device, stream):
    ...
    try:
        ...
        if dmax - dmin > 1e-6:
            normalized = (depth_map_filtered - dmin) / (dmax - dmin)
        else:
            normalized = np.zeros_like(depth_map_filtered)
        return normalized
    except Exception as e:
...
def run_object_detection(model, frame_bgr, device, stream):
    ...
    try:
        ...
        for _, row in df.iterrows():
            if row['confidence'] > 0.4: # Confidence threshold
        ...
        print(f"Object detection error: {type(e).__name__} - {e}")
        return [], None


def process_visual_odometry(prev_gray, curr_gray, prev_kp, prev_des, K):
    """ Processes one step of visual odometry using ORB features. """
    # --- Increased features, relaxed RANSAC threshold ---
    ...
    try:
        ...
        good_matches = matches[:max(int(len(matches) * 0.7), 30)] # Keep more good matches

        if len(good_matches) < 8:
            # print(f"Not enough good matches ({len(good_matches)})") # Can be verbose
            return kp, des, R_rel, t_rel, pts1, pts2
        ...
        # --- Relaxed threshold ---
        E, mask_e = cv2.findEssentialMat(pts2, pts1, K, method=cv2.RANSAC, prob=0.999, threshold=1.5) # Relaxed threshold

        if E is None or mask_e is None:
            # print("Essential matrix computation failed.")
            return kp, des, R_rel, t_rel, pts1, pts2

        # Pass essential matrix mask to recoverPose for potentially better results
        _, R_rec, t_rec, mask_p = cv2.recoverPose(E, pts2, pts1, K, mask=mask_e)

        if R_rec is None or t_rec is None or not (np.isfinite(R_rec).all() and np.isfinite(t_rec).all()):
            # print("Pose recovery failed or produced invalid values.")
            return kp, des, R_rel, t_rel, pts1, pts2

        inlier_mask = mask_p.ravel() == 1
        pts1_in = pts1[inlier_mask]
        pts2_in = pts2[inlier_mask]
        num_inliers = np.sum(inlier_mask)

        min_inliers_for_motion = 8 # Require a decent number of inliers for robust motion estimate
        if num_inliers < min_inliers_for_motion:
            # Print only if VO frequently fails
            # print(f"Not enough inliers after pose recovery ({num_inliers}), skipping VO update.")
            return kp, des, R_rel, t_rel, pts1, pts2 # Return previous estimate (no motion)

        # Motion estimate is valid
        R_rel = R_rec
        t_rel = t_rec
        pts1, pts2 = pts1_in, pts2_in

        # --- Debug Print for successful VO translation ---
        # print(f"VO OK ({num_inliers} inliers). t_rel: [{t_rel[0,0]:.3f}, {t_rel[1,0]:.3f}, {t_rel[2,0]:.3f}]")
...
# --- Path Planning (Modified with Depth Threshold) ---
def plan_trajectory(objects, current_pos_img, depth_map, frame_shape,
                    obstacle_depth_threshold=0.6):
    # ... (no changes needed in this function, using version from previous response) ...
    h, w = frame_shape[:2]
    ...
    for obj in objects:
        ...
        is_obstacle = False
        if obj_depth is not None and obj_depth < obstacle_depth_threshold:
             is_obstacle = True
             num_depth_obstacles += 1
        elif obstacle_depth_threshold >= 1.0 and obj.get('bbox'):
             is_obstacle = True
    ...
    cost_vis[obstacle_map == 255] = [0, 0, 200] # Dark Red for obstacle zones
    ...
    if not (0 <= start_node[0] < w and 0 <= start_node[1] < h) or obstacle_map[start_node[1], start_node[0]] != 0:
        # Don't print warning every frame if stuck, maybe only occasionally
        # print(f"Warning: Start node {start_node} is invalid or inside an obstacle region!")
        return [start_node], obstacle_map, cost_vis # Return just start node
...
def main():
    ...
    new_K = K.copy()
    # --- Changed map origin and scale factor ---
    map_origin_vis = (fw // 2, fh * 3 // 4) # Start lower on map
    ...
    vo_scale_factor = 40.0 # Adjusted scale factor - TUNE THIS!
    ...
    try:
        while True:
            ...
            if frame_count == 0:
                ...
                print("Optimal New Camera Matrix (new_K) applied.") # Simplified print

            frame_undistorted = cv2.undistort(frame_bgr, K, dist, None, new_K)
            # Get actual height/width *after* undistortion/cropping if ROI is used
            ...
            def vo_task(): vo_result[:] = process_visual_odometry(prev_gray, img_gray, prev_kp, prev_des, new_K)
            def depth_task(): depth_result[0] = run_depth_estimation(midas_model, midas_transform, img_rgb, device, stream_midas)
            def obj_task(): obj_result[:] = run_object_detection(yolo_model, frame_undistorted, device, stream_yolo)
            thread_vo = threading.Thread(target=vo_task); thread_depth = threading.Thread(target=depth_task); thread_obj = threading.Thread(target=obj_task)
            ...
            if device == 'cuda':
                ...
                # --- Add cache clearing ---
                # torch.cuda.empty_cache() # Uncomment if GPU memory errors persist, may slow down FPS
            ...
            # --- Update Visual Odometry State ---
            if np.linalg.norm(t_rel) > 1e-6: # Update only if significant motion detected
                t_update = R_cum @ (vo_scale_factor * t_rel)
                # Optional: Zero out vertical drift if camera assumed level
                # t_update[1,0] = 0
                t_cum += t_update
            R_cum = R_cum @ R_rel

            # --- Calculate and Clip Visualization Coordinates ---
            vis_x_raw = map_origin_vis[0] + t_cum[0, 0]
            vis_y_raw = map_origin_vis[1] - t_cum[2, 0] # Use Z component, negate for forward=down in vis
            vis_x = int(np.clip(vis_x_raw, 0, w - 1)) # Clip before converting to int
            vis_y = int(np.clip(vis_y_raw, 0, h - 1)) # Clip before converting to int
            ...
            planned_waypoints, obstacle_map_vis, cost_map_vis = plan_trajectory(
                objects_with_depth,
                current_pos_vis, # Use the clipped visual position
                depth_map_normalized,
                frame_undistorted.shape,
                obstacle_depth_threshold=0.6 # TUNABLE
            )

            nav_instructions = generate_instructions(planned_waypoints, step_size=15, angle_threshold=40) # Ensure threshold matches
            ...
            # --- Update state ---
            prev_gray = img_gray.copy(); prev_kp = kp; prev_des = des
            frame_count += 1

            if cv2.waitKey(1) & 0xFF == ord('q'): print("Exit requested."); break

    finally:
        print("Releasing resources...")
        cap.release(); cv2.destroyAllWindows()
        if tts_manager.is_alive():
            tts_manager.stop(); tts_manager.join(timeout=2.0)
            if tts_manager.is_alive(): print("Warning: TTS thread did not exit cleanly.")
